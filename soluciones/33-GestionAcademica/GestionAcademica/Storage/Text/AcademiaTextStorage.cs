...
public class AcademiaTextStorage : IAcademiaTextStorage {
    ...
    public void Salvar(IEnumerable<Persona> items, string path) {
        ...
        // NOTA PARA EL ALUMNO: Evitamos .ToList().ForEach() para no crear una lista intermedia.
        // Recorremos el flujo de datos (IEnumerable) y escribimos conforme recibimos.
        foreach (var p in items) writer.WriteLine(ObteneLineaDePersona(p));
    }
