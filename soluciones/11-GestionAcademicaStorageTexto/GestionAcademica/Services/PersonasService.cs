...
public class PersonasService(
    ...
    public int ExportarDatos() {
        try {
            ...
            _logger.Information("Exportando datos a almacenamiento externo. Total personas: {count}.", personas.Count);
    ...
    public int ImportarDatos() {
        try {
            ...
            var personas = storage.Cargar(Configuracion.AcademiaFile).ToList();
            if (personas.Count == 0) {
                _logger.Warning("No se encontraron datos para importar en el archivo {file}.",
                    Configuracion.AcademiaFile);
                throw new Exception(
                    $"No se encontraron datos para importar en el archivo {Configuracion.AcademiaFile}.");
            }

            // Eliminamos lo que haya en la base de datos para evitar duplicados (en un caso real, haríamos un merge o actualización).
            //repository.GetAll().ToList().ForEach(p => repository.Delete(p.Id));
            repository.DeleteAll();
            // Guardamos cada persona en la base de datos.
            personas.ForEach(p => Save(p));
            _logger.Information("Datos importados correctamente. Total personas: {count}", personas.Count);
            return personas.Count;
        }
        catch (Exception ex) {
