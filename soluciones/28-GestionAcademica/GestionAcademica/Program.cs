...
using GestionAcademica.Factories.Storages;
...
void Main() {
    ...
    // NOTA PARA EL ALUMNO: Ahora el storage y repositorio se inyectan dinámicamente según el appsettings.json.
    ...
    // Metemos los datos de prueba solo si no es repositorio binario (para no perder datos)
        repository.DeleteAll();
        PersonasFactory.Seed().ToList().ForEach(p => service.Save(p));


    OpcionMenu opcion;
    const string RegexOpcionMenu = @"^([0-9]|1[0-7])$";
...
void ImportarDatos(IPersonasService service) {
    ...
    if (!PedirConfirmacion(
            $"Desea importar los datos desde el fiche o: {Configuracion.AcademiaFile}\nEsta acción puede sobrescribir datos existentes. ¿Desea continuar?")) {
        WriteLine("👋 Operación cancelada.");
        return;
    }
...
void RealizarBackup(IPersonasService service) {
...
void RestaurarBackup(IPersonasService service) {
    ...
    try {
        ...
        WriteLine($"✅ Restauración completada. Total registros: {restaurados}");
    }
    catch (Exception ex) {
        WriteLine($"☠️ ERROR AL RESTAURAR: {ex.Message}");
    }
}
